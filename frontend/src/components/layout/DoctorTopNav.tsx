...
import { LogOut, ChevronLeft, Bell, Info, CheckCircle2, AlertTriangle, XCircle, ShieldCheck, Activity } from "lucide-react"
...
const pageTitles: Record<string, string> = {
 ...
 "/doctor/medications": "Clinical Medications",
 "/doctor/settings": "Settings",
 "/doctor/profile": "Provider Profile",
}
...
export function DoctorTopNav() {
 ...
 const title = Object.entries(pageTitles).find(([path]) => pathname === path || (path !== "/doctor/dashboard" && pathname.startsWith(path)))?.[1] || "CareFlow AI"
  ...
  <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-card flex items-center justify-between px-6 sticky top-0 z-10 shadow-sm transition-colors duration-300">
   <div className="flex items-center gap-6">
    <div className="flex flex-col">
     ...
     <p className="text-xs text-slate-500 font-medium">{greeting}, {user?.name?.split(" ")[0] || "Dr."}</p>
    </div>
   ...
   <div className="flex items-center gap-4">
    ...
    <Popover>
     <PopoverTrigger aria-label="Notifications" className="relative p-2 text-slate-500 hover:text-sky-500 hover:bg-sky-50 dark:hover:bg-sky-900/20 rounded-xl transition-all duration-300 group">
      ...
      <AnimatePresence>
       {_hasHydrated && notifications.filter(n => !n.isRead).length > 0 && (
        <motion.span
         initial={{ scale: 0 }}
         animate={{ scale: 1 }}
         exit={{ scale: 0 }}
