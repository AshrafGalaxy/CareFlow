...
export function InteractiveBot() {
  ...
  // 2. Click Activation Sequence
  const handleClick = () => {
    if (isActive) return
    setIsActive(true)
    setTimeout(() => {
      setIsActive(false)
    }, 3600) // 1.2s wave * 3 = 3.6s
  }
  ...
  return (
    ...
    >
      <div
        ref={containerRef}
        className={cn(
          "relative group w-16 h-16 transition-transform duration-100 ease-out preserve-3d animate-ambient-float",
          isActive && "animate-head-tilt"
        )}
        style={{
      ...
      >
        {/* Glow behind bot */}
        <div className="absolute inset-0 bg-sky-400 rounded-full blur-[20px] opacity-30 group-hover:opacity-60 transition-opacity duration-500" />
        ...
        <div className="relative w-full h-full" style={{ transform: "translateZ(20px)" }}>
          <style dangerouslySetInnerHTML={{__html: `
            ...
            .bot-cross.active {
              animation: vital-pulse 0.4s ease-in-out infinite alternate;
            }
            ...
            .bot-smile.active {
              animation: smile-bounce 0.8s ease-in-out infinite alternate;
            }

            /* Arm & Leg Swings */
            .bot-arm-left { transform-origin: 270px 580px; }
            .bot-arm-right { transform-origin: 990px 580px; }
            .bot-leg-left { transform-origin: 490px 1010px; }
            .bot-leg-right { transform-origin: 770px 1010px; }

            .bot-arm-left.active { animation: swing-arm-left 0.6s ease-in-out infinite alternate; }
            .bot-arm-right.active { animation: swing-arm-right 0.6s ease-in-out infinite alternate; }
            .bot-leg-left.active { animation: swing-leg-left 0.6s ease-in-out infinite alternate; }
            .bot-leg-right.active { animation: swing-leg-right 0.6s ease-in-out infinite alternate; }
            ...
            @keyframes vital-pulse {
              ...
              to { filter: drop-shadow(0 0 15px #05BF75) brightness(1.5); }
            }
            @keyframes smile-bounce {
              0% { transform: scaleY(1); }
              100% { transform: scaleY(2.5); stroke-width: 25px; filter: drop-shadow(0 0 8px rgba(5,191,117,0.8)); }
            }
          ...
          <svg viewBox="150 200 950 1000" className="w-full h-full drop-shadow-xl overflow-visible">
            ...
            {/* Custom Drawn Legs */}
            <rect className={cn("bot-leg-left", isActive && "active")} x="450" y="980" width="80" height="180" rx="40" fill="#FEFEFE" stroke="#0898F7" strokeWidth="15" />
            <rect className={cn("bot-leg-right", isActive && "active")} x="730" y="980" width="80" height="180" rx="40" fill="#FEFEFE" stroke="#0898F7" strokeWidth="15" />

            {/* Custom Drawn Arms */}
            <rect className={cn("bot-arm-left", isActive && "active")} x="240" y="550" width="60" height="220" rx="30" fill="#0897F7" />
            <rect className={cn("bot-arm-right", isActive && "active")} x="960" y="550" width="60" height="220" rx="30" fill="#0998F6" />
        ...
        <div 
          className={cn(
            "absolute bottom-full right-0 mb-4 w-56 bg-white text-slate-700 text-sm font-medium py-3 px-4 rounded-xl shadow-xl border border-slate-100 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none origin-bottom-right scale-95 group-hover:scale-100",
            isActive && "opacity-100 scale-100"
          )}
          style={{ transform: "translateZ(50px)" }}
        >
          {isActive ? "Hi! How can I help you today?" : "I'm your AI Health Companion"}
          {/* Tooltip triangle */}
