...
export default function PatientAppointmentsPage() {
  ...
  const confirmedFollowUps = followUps?.filter(f => f.status === 'confirmed' || f.status === 'requested') || [];
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      {isLoading ? (
        ...
        <div className="space-y-8">
          ...
          {/* Upcoming & Requested */}
          <div className="space-y-4">
            <h2 className="text-lg font-semibold text-foreground">Upcoming & Requested</h2>
            {confirmedFollowUps.length === 0 ? (
              <p className="text-muted-foreground text-sm py-4">No upcoming appointments.</p>
            ) : (
              <div className="grid gap-4">
                {confirmedFollowUps.map(fu => (
                  <div key={fu.id} className="bg-card border border-border rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 hover:border-sky-200 dark:hover:border-sky-800 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-sky-50 dark:bg-sky-900/20 text-sky-500 flex items-center justify-center shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={cn(
                        ...
                        fu.status === 'confirmed' ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-400"
                      )}>
                        {fu.status}
                      </span>
                    </div>
