...
import { FileText, Plus, Search, Calendar, ChevronRight, MessageSquare, Eye, Trash2 } from 'lucide-react'
 ...
 const handleDeleteReport = async (reportId: string) => {
  try {
   ...
   toast("Report Deleted", {
    description: "Your lab report has been permanently removed.",
    icon: <Trash2 className="h-4 w-4 text-destructive" />,
   })
   useNotificationStore.getState().addNotification({
    title: "Report Deleted",
    message: "Your lab report was successfully deleted from the cloud.",
    type: "info"
   })
  } catch (err) {
 ...
 return (
  <div className="w-full space-y-8">
   ...
   <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
     <h1 className="text-2xl font-heading font-bold text-foreground tracking-tight">My Reports</h1>
     <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Manage and analyze your medical laboratory reports</p>
    </div>
    <Link 
     href="/reports/upload" 
   ...
   <div className="bg-card rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
    ...
    {isLoading ? (
     ...
     <div className="p-16 text-center">
      <div className="h-16 w-16 bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
     ...
     <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
      {filteredReports.map((report) => (
       ...
       >
        <div className="h-12 w-12 bg-sky-100 text-sky-600 dark:bg-sky-900/30 dark:text-sky-400 rounded-xl flex items-center justify-center shrink-0 mr-4 group-hover:scale-105 transition-transform">
         <FileText size={24} />
        </div>
        ...
        <div className="flex-1 min-w-0 pr-4">
         ...
         <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500 dark:text-slate-400">
          <span className="flex items-center gap-1.5">
           <Calendar size={14} />
           {new Date(report.uploaded_at).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' })}
          </span>
          <span className="hidden sm:inline text-slate-300 dark:text-slate-600">•</span>
          {report.processing_status === 'done' ? (
           <span className="text-emerald-600 font-medium bg-emerald-50 dark:bg-emerald-900/30 dark:text-emerald-400 px-2 py-0.5 rounded text-xs">Analyzed by AI</span>
          ) : report.processing_status === 'failed' ? (
           <span className="text-red-600 font-medium bg-red-50 dark:bg-red-900/30 dark:text-red-400 px-2 py-0.5 rounded text-xs">Processing Failed</span>
          ) : (
           <span className="text-amber-600 font-medium bg-amber-50 dark:bg-amber-900/30 dark:text-amber-400 px-2 py-0.5 rounded text-xs animate-pulse">Processing...</span>
          )}
         </div>
        </div>
        ...
        <div className="flex items-center gap-3">
         <button
          ...
          className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 hover:bg-slate-200 border border-slate-200 rounded-lg text-xs font-semibold transition-colors dark:bg-slate-800 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700"
         >
          <Eye size={14} /> View
         </button>
         <button
         ...
         <div onClick={(e) => e.stopPropagation()} className="ml-2">
          <ConfirmDialog
           title="Delete Report"
           description="Are you sure you want to delete this lab report? This will permanently delete the file from cloud storage and remove it from your records."
           isDestructive={true}
           confirmText="Delete"
           ...
           trigger={
            <button
             type="button"
             className="opacity-0 group-hover:opacity-100 p-2 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-lg transition-all flex items-center"
            >
             <Trash2 size={16} />
            </button>
           }
