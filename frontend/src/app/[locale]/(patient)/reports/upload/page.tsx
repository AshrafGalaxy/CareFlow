   ...
   <div className="grid lg:grid-cols-3 gap-6">
    ...
    <div className="lg:col-span-2 bg-card rounded-2xl border border-border shadow-sm p-8">
     ...
     {isIdle && (
      <div
       ...
       className={`
        ...
        ${isError ? "border-red-400 bg-red-50/50 dark:bg-red-950/20 animate-shake" : isHover ? "border-sky-500 bg-sky-50/50 dark:bg-sky-950/20" : "border-border bg-muted/50 hover:border-slate-400 dark:hover:border-slate-600"}
       `}
      >
       <input {...getInputProps()} />
       {isError ? (
        <>
         ...
         <p className="text-xs text-muted-foreground mt-3">Click or drag to try again</p>
        </>
       ) : (
        <>
         <UploadCloud className={`h-14 w-14 mb-4 transition-colors duration-200 ${isHover ? "text-sky-500" : "text-muted-foreground"}`} />
         <p className={`text-base font-semibold mb-1 transition-colors ${isHover ? "text-sky-600 dark:text-sky-400" : "text-foreground"}`}>
          {isHover ? "Drop your file here!" : "Drag your medical records here"}
         </p>
         {!isHover && <p className="text-sm text-muted-foreground mb-5">or click to browse files from your device</p>}
         <div className="flex gap-2 flex-wrap justify-center">
          {ACCEPTED_FILE_LABELS.map(f => (
           <span key={f} className="text-xs font-medium bg-muted text-muted-foreground px-3 py-1 rounded-full">{f}</span>
          ))}
         </div>
         <p className="text-xs text-muted-foreground mt-4">Maximum file size: {MAX_FILE_SIZE_LABEL}</p>
        </>
       )}
      </div>
     )}
     ...
     {status === "selected" && file && (
      <div className="space-y-6">
       <div className="flex items-center gap-4 p-5 border border-border rounded-xl bg-muted/50">
        <div className="h-12 w-12 rounded-xl bg-red-50 dark:bg-red-950/30 flex items-center justify-center shrink-0">
        ...
        <button onClick={removeFile} aria-label="Remove file" className="p-1.5 text-muted-foreground hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors">
         <X className="h-4 w-4" />
        </button>
       </div>
       <button
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm p-6">
       ...
       <ul className="space-y-2.5 text-sm">
        ...
        {["Excel / Word documents"].map(item => (
         <li key={item} className="flex items-center gap-2 text-muted-foreground/50 line-through">
     ...
     <div className="bg-sky-50/50 dark:bg-sky-950/20 rounded-2xl border border-sky-100/50 dark:border-sky-900/50 p-6">
      <div className="flex items-center gap-2 mb-3">
       <Info className="h-4 w-4 text-sky-500 shrink-0" />
       <h3 className="text-sm font-semibold text-sky-700 dark:text-sky-400">Tips for best results</h3>
      </div>
