...
import { Eye, EyeOff, Loader2, AlertCircle, UserCircle2, Stethoscope, CheckCircle } from "lucide-react"
...
export default function RegisterPage() {
 ...
 return (
  <div className="w-full max-w-[440px]">
   ...
   {/* Card */}
   <div className="bg-card rounded-2xl shadow-lg border border-slate-100 dark:border-slate-800 p-10">
    <div className="mb-6">
    ...
    <div
     ...
     className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl gap-1 mb-6"
    >
     <button
      type="button"
      ...
      className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 ${
       role === "patient"
        ? "bg-card text-sky-600 shadow-sm"
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
     ...
     <div className="space-y-1.5">
      ...
      <input
       ...
       className={`w-full h-12 px-4 rounded-lg border text-foreground text-sm placeholder:text-slate-400 outline-none transition-all duration-200 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 ${
        errors.name ? "border-red-400 bg-red-50 dark:bg-red-950/50" : "border-slate-200 dark:border-slate-700 bg-card"
       }`}
       {...register("name", {
     ...
     {/* Email */}
     <div className="space-y-1.5">
      <label htmlFor="reg-email" className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wide">
       Email Address
      </label>
      <input
       id="reg-email"
       ...
       className={`w-full h-12 px-4 rounded-lg border text-foreground text-sm placeholder:text-slate-400 outline-none transition-all duration-200 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 ${
        errors.email ? "border-red-400 bg-red-50 dark:bg-red-950/50" : "border-slate-200 dark:border-slate-700 bg-card"
       }`}
       {...register("email", {
     ...
     {/* Password */}
     <div className="space-y-1.5">
      <label htmlFor="reg-password" className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wide">
       Password
      </label>
      <div className="relative">
       <input
        ...
        className={`w-full h-12 px-4 pr-12 rounded-lg border text-foreground text-sm placeholder:text-slate-400 outline-none transition-all duration-200 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 ${
         errors.password ? "border-red-400 bg-red-50 dark:bg-red-950/50" : "border-slate-200 dark:border-slate-700 bg-card"
        }`}
        {...register("password", {
     ...
     <div className="space-y-1.5">
      ...
      <div className="relative">
       <input
        ...
        className={`w-full h-12 px-4 pr-12 rounded-lg border text-foreground text-sm placeholder:text-slate-400 outline-none transition-all duration-200 focus:ring-2 focus:ring-sky-500 focus:border-sky-500 ${
         errors.confirmPassword ? "border-red-400 bg-red-50 dark:bg-red-950/50" : "border-slate-200 dark:border-slate-700 bg-card"
        }`}
